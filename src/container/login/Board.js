...
function Board(el, width, height, options) {
  options = options || {}
  this.el = el
  this.width = width
  this.height = height
  this.dotCount =
    options.dotCount || Math.floor((width * height) / Math.pow(120, 2))
...
Board.prototype = {
  ...
  makeDots() {
    ...
    for (let i = 0; i < dotCount; i++) {
      dots.push(
        new Dot(this, width * Math.random(), height * Math.random(), options)
      )
    }
  },
